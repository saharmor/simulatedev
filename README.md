...
- macOS only: SimulateDev currently only works on macOS. Windows and Linux support is planned for future releases.
...
Clone this repository → Create a virtual environment → Install requirements.txt → Copy .env.example to .env and replace API keys → Run the following command (replace with your coding IDE of choice and GitHub repository URL):
```bash
