# SimulateDev
Automate cutting-edge AI coding IDEs such as Cursor, Devin, and Claude Code to implement features and fixes.
...
- **macOS only**: SimulateDev currently works only on macOS. Windows and Linux support are planned for future releases.
...
- SimulateDev will periodically bring the IDE to the front to monitor the coding agent's progress and state. Ideally, you can take a coffee break and let it work!
...
4. Copy .env.example to .env and replace API keys
5. Run the following command (replace with your coding IDE of choice and GitHub repo URL):
```bash
...
The timeout is automatically validated and clamped to reasonable bounds (30-second minimum, 2-hour maximum).
...
2. **Smart Workflow**: If no push access is available, automatically forks the repository
