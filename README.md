# SimulateDev
Automate cutting-edge AI coding IDEs such as Cursor, Devin, and Claude Code with custom prompts.
...
- **IDE Prerequisites**: Ensure your coding IDEs (Cursor, Windsurf, Claude Code) are already installed and ready to use. SimulateDev doesn't handle IDE setup or installation.
- **Windsurf Users**: Turn on **Turbo mode** for terminal auto-execution to ensure commands run automatically. [Learn more](https://www.reddit.com/r/windsurf/comments/1kksxgh/comment/mrx2el2/)
- **Monitoring**: SimulateDev will periodically bring the IDE to the front to monitor the coding agent's progress and state. Ideally, you can take a coffee break and let it work.
...
Clone this repo → Create virtual environment → Install requirements.txt → Copy .env.example to .env and replace API keys → Run the following command (replace with your coding IDE of choice and GitHub repo URL):
```bash
...
- **IDE Integration**: SimulateDev programmatically opens and controls your installed coding IDEs through:
  ...
  - **Headless Operation**: Claude Code runs without GUI requirements
