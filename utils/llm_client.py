...
class CommitPRContent(BaseModel):
    ...
    concise_task_description: str = Field(description="Concise 1-2 sentence task description for PR")
...
6. A concise task description (1-2 sentences max) for the PR Task section
...
For the concise_task_description field, create a CONCISE task summary (1-2 sentences max):
This concise task description will be displayed in the PR's "Task" section, so keep it simple and focused.
    ...
    try:
        ...
        if result:
            return {
                ...
                "concise_task_description": result.concise_task_description
            }
        else:
